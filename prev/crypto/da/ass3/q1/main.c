...
uint64_t des(uint64_t input, uint64_t key) {
    ...
    for (i = 0; i< 16; i++) {
        ...
        /* same as initil permutation without tmp variable */
        for (j = 0; j < 48; j++) {
    ...
    /* Initial key split: C_(0) and D_(0)*/
...
int main(int argc, const char * argv[]) {
    ...
    uint64_t key = 0x00A00A01A00A0100;
                
    printf ("Input: %016llx, Key: %llx\n", input, key);
